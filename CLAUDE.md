...
Guide for creating effective, portable skills that extend Claude''s

- `.claude/skills/agent-skill-creator/SKILL.md`
- `.claude/skills/agent-skill-creator/references/description-authoring.md`: The `description` field is the primary triggering mechanism. Claude reads it to decide when to activate a skill. A weak description means the skill never triggers — or triggers for the wrong tasks.
- `.claude/skills/agent-skill-creator/references/output-patterns.md`: Use these patterns when skills need to produce consistent, high-quality output.
- `.claude/skills/agent-skill-creator/references/testing-iteration.md`: Verify the skill activates (and doesn't activate) correctly.
- `.claude/skills/agent-skill-creator/references/troubleshooting.md`: **Symptoms**: Skill is ignored for relevant tasks. User must explicitly invoke it.
- `.claude/skills/agent-skill-creator/references/validation-checklist.md`: Every skill must meet these structural requirements. Validate each item before considering a skill complete.
- `.claude/skills/agent-skill-creator/references/workflows.md`: For tasks with strict step ordering, dependencies, and validation gates.
...
- `.claude/skills/core-coding-standards/SKILL.md`
- `.claude/skills/core-coding-standards/rules/_sections.md`
- `.claude/skills/core-coding-standards/rules/comment-why-not-what.md`: Self-documenting code is the goal. Comments are a last resort, not a first choice. When you do comment, explain the *why* (intent, trade-offs, workarounds), never the *what* (the code already says what it does).
- `.claude/skills/core-coding-standards/rules/org-no-premature-abstraction.md`: Don't create abstractions until you need them. Concrete code that works beats elegant abstractions that don't.
...
- `.claude/skills/tech-drizzle/SKILL.md`
- `.claude/skills/tech-drizzle/rules/_sections.md`: This file defines all sections, their ordering, impact levels, and descriptions. The section ID (in parentheses) is the filename prefix used to group rules.
- `.claude/skills/tech-drizzle/rules/migration-push-vs-generate.md`: Use `db:push` for rapid local iteration. Generate migrations only when ready to commit.
- `.claude/skills/tech-drizzle/rules/migration-review-sql.md`: Drizzle-kit infers migrations from schema diffs. Always review the generated SQL before committing.
- `.claude/skills/tech-drizzle/rules/migration-transaction-safety.md`: Wrap multi-operation writes in `db.transaction()` to ensure atomicity. All operations commit together or all rollback together, preventing partial updates and maintaining referential integrity. Use nested transactions with savepoints for fine-grained control.
- `.claude/skills/tech-drizzle/rules/qb-prepared-statements.md`: Use `.prepare()` with `sql.placeholder()` to pre-compile queries and execute them multiple times with different parameters. Prepared statements provide significant performance improvement for queries that run frequently in loops or hot paths.
- `.claude/skills/tech-drizzle/rules/qb-use-for-aggregations.md`: The SQL-like query builder (`db.select`) is better for aggregations, GROUP BY, and complex queries.
- `.claude/skills/tech-drizzle/rules/rqb-prefer-relational.md`: Use `db.query` API for reads with relations. It's more readable and handles JOINs automatically.
- `.claude/skills/tech-drizzle/rules/rqb-select-columns.md`: Use the `columns` option to fetch only needed fields. Avoid returning entire rows.
- `.claude/skills/tech-drizzle/rules/rqb-with-eager-loading.md`: Use the `with` clause to eagerly load relations in a single query. This prevents the N+1 query problem where you make one query for the initial data, then N additional queries for related records.
- `.claude/skills/tech-drizzle/rules/schema-export-order.md`: In your schema index, export tables before their dependents, and relations last.
- `.claude/skills/tech-drizzle/rules/schema-infer-types.md`: Export `$inferSelect` and `$inferInsert` types for each table. Use them consistently across the codebase.
- `.claude/skills/tech-drizzle/rules/schema-one-table-per-file.md`: Define each table in its own file. Keep relations in a separate file to avoid circular imports.

## Elysia.js Skill

Build type-safe Bun/Node.js HTTP servers with Elysia and consume them with Eden's end-to-end type-safe client. Use when users mention Elysia, ElysiaJS, elysia server, bun backend, Eden Treaty, eden fetch, @elysiajs/eden, or need to create routes, plugins, middleware, validation, or a type-safe cl...

- `.claude/skills/tech-elysia/SKILL.md`
- `.claude/skills/tech-elysia/references/eden.md`: Eden connects a typed Elysia server to the client with zero code generation.
- `.claude/skills/tech-elysia/references/lifecycle.md`: Hooks apply only to routes registered **after** them:
- `.claude/skills/tech-elysia/references/plugins.md`: Any Elysia instance can be a plugin:
- `.claude/skills/tech-elysia/references/validation.md`
...
## Backend Architecture — Clean Architecture + Inversify + Elysia

Generate boilerplate for this project's backend: modules, use cases, repositories, DTOs, entities, routes, and Inversify bindings. Triggers on "new module", "add use case", "create repository", "add route", or any backend feature creation in apps/api.
